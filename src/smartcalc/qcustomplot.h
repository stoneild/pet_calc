...
class QCP_LIB_DECL QCPScatterStyle {
  ...
  */
  ...
  /*!
    ...
    function. Scatters are drawn with the pen and brush specified with \ref
...
/*! \class QCPDataContainer
  ...
  non-const iterators can be used (\ref begin,
  \ref end). Changing data members that are not the sort key (for most data
  types called \a key) is safe from the container's perspective.
...
/*!
  ...
  such that a full resort is never necessary. However, if you choose to directly
  manipulate the sort key on data points by accessing and modifying it through
  the non-const iterators (\ref begin, \ref end), it is your responsibility to
  bring the container back into a sorted state before any other methods are
  called on it. This can be achieved by calling this method immediately after
  finishing the sort key manipulation.
*/
template <class DataType>
...
/*! \fn virtual int QCPPlottableInterface1D::findBegin(double sortKey, bool
  ...
  will be considered, otherwise the one just above.
...
/*! \class QCPAbstractPlottable1D
  ...
  this base class is to provide the member
  \a mDataContainer (a shared pointer to a \ref QCPDataContainer
  "QCPDataContainer<DataType>") and implement the according virtual methods of
  the \ref QCPPlottableInterface1D, such that most subclassed plottables don't
  ...
  method and need to draw selected segments with a different pen/brush than
...
class QCP_LIB_DECL QCPGraph : public QCPAbstractPlottable1D<QCPGraphData> {
 ...
 public:
  /*!
    ...
    is drawn with the current pen of the graph (\ref setPen).
    \see setLineStyle
  */
  enum LineStyle {
...
class QCP_LIB_DECL QCPCurve : public QCPAbstractPlottable1D<QCPCurveData> {
 ...
 public:
  /*!
    ...
    is drawn with the current pen of the curve (\ref setPen).
    \see setLineStyle
  */
  enum LineStyle {
